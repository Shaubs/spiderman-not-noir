...
# Paths
SAMPLES_DIR = "data_creation_lab/samples"
MODEL_OUTPUT = "random_forest_classifier/model.pkl"
...
def load_all_samples():
    ...
    all_files = glob.glob(os.path.join(SAMPLES_DIR, "*.json"))
    
    positive_files = [f for f in all_files if "spiderman" in os.path.basename(f).lower()]
    negative_files = [f for f in all_files if "negative" in os.path.basename(f).lower()]
...
def main():
    ...
    print(f"  python random_forest_classifier/run_classifier.py")
